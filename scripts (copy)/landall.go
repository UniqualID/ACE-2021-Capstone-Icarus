...
func main() {
	...
	var query = icarus.NewQuery("10.59.144.32", "279")
	...
	for !inRange {
		...
		if distance > 2000 {
			...
			homeCmd := icarus.AddCmd(nil, icarus.GOTO, destLat, destLon, destAlt, 90, 0, 0, 0)
			...
			homeCmd := icarus.AddCmd(nil, icarus.GOTO, destLat, destLon, destAlt, 50, 0, 0, 0)
			gotoSeq = query.Goto(int(vehID), homeCmd)
			responseChan, _ = query.Execute()
			response = <-responseChan
			_, ok = response.Get(gotoSeq)
			...
			homeCmd := icarus.AddCmd(nil, icarus.GOTO, destLat, destLon, destAlt, 20, 0, 0, 0)
			gotoSeq = query.Goto(int(vehID), homeCmd)
			responseChan, _ = query.Execute()
			response = <-responseChan
			_, ok = response.Get(gotoSeq)
