//addVehicle.go
//This file contains an example of adding a new vehicle to Icarus Server
package main

import (
	"fmt"

	icarus "git.ironzone.ace/icarus/icarusClient"
)

const (
	defaultCert string = `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
`
	defaultKey string = `-----BEGIN RSA PRIVATE KEY-----

aJWMolGAClXedrb1jV2zZgl9xwi8kG1Y9EL4uVuCz2k03dFGuznMTW2v
-----END RSA PRIVATE KEY-----
`
)

var vehIP string
var vehID string
var vehc3poPort string
var vehCallsign string
var vehDaedalusPort string

func main() {

	// User enters vehicle ID (vehID) of drone
	fmt.Println("Enter the IP address of the drone:")
	fmt.Scanln(&vehIP)

	// User enters C3PO_Port associated with drone
	fmt.Println("Enter the C3PO_Port of the drone:")
	fmt.Scanln(&vehc3poPort)

	// User enters desired end state longitude
	fmt.Println("Enter the callsign of the drone:")
	fmt.Scanln(&vehCallsign)

	// User enters desired end state longitude
	fmt.Println("Enter the calDaedalus_Port of the drone:")
	fmt.Scanln(&vehDaedalusPort)

	//Create a new query pointed at the IcarusServer instance
	query := icarus.NewQuery("10.59.144.202", "179")

	//Add new vehicle to IcarusServer
	addSeq := query.AddNewVehicle(vehIP, vehc3poPort, vehCallsign, vehDaedalusPort, 0, make([]string, 1), []byte(defaultCert), []byte(defaultKey), 0, icarus.DefaultC3poTime, icarus.DefaultDaedalusTime)

	//Authenticate to the server
	resp, ok := query.Authenticate("valinar", "gitlabsux;(bloodybreach")
	if !ok {
		fmt.Println("Unable to authenticate to IcarusServer:", resp)
	}

	//Uncomment this line to show the JSON query being sent to the server
	//query.ShowQuery()

	//Execute the query and read the responses
	responseChan, _ := query.Execute()

	fmt.Println("Waiting for responses:")
	response := <-responseChan
	fmt.Println(response.Get(addSeq))

	//Uncomment this line to show the JSON response returned by the server
	//response.ShowResponse()
}
