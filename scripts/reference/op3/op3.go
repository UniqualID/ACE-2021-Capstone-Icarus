package main

import (
	"encoding/csv"
	"fmt"
	"io"
	"log"
	"os"
	"strconv"
	"time"

	icarus "git.ironzone.ace/icarus/icarusClient"
)

const (
	defaultCert string = `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
`
	defaultKey string = `-----BEGIN RSA PRIVATE KEY-----

aJWMolGAClXedrb1jV2zZgl9xwi8kG1Y9EL4uVuCz2k03dFGuznMTW2v
-----END RSA PRIVATE KEY-----
`
)

type GPS struct {
	lat float64
	lon float64
}

type Sortie struct {
	gps []GPS
}

const MAX_VEL = 120.0
const ALT_DIF = 200

const JOHNNY = 2
const FRANCIS = 1
const BABY = 0

var query0 = icarus.NewQuery("10.59.144.202", "179")
var query1 = icarus.NewQuery("10.59.144.202", "179")
var query2 = icarus.NewQuery("10.59.144.202", "179")

func main() {
	// Authentication
	resp, ok := query0.Authenticate("valinar", "gitlabsux;(bloodybreach")
	if !ok {
		fmt.Print((time.Now()).Format("15:04:05"), " ")
		fmt.Println("Unable to authenticate to IcarusServer:", resp)
		os.Exit(1)
	}
	resp, ok = query1.Authenticate("valinar", "gitlabsux;(bloodybreach")
	if !ok {
		fmt.Print((time.Now()).Format("15:04:05"), " ")
		fmt.Println("Unable to authenticate to IcarusServer:", resp)
		os.Exit(1)
	}
	resp, ok = query2.Authenticate("valinar", "gitlabsux;(bloodybreach")
	if !ok {
		fmt.Print((time.Now()).Format("15:04:05"), " ")
		fmt.Println("Unable to authenticate to IcarusServer:", resp)
		os.Exit(1)
	}

	// Add vehicles from assets.csv
	addVehicles()

	// Print status
	status_all()

	// Get active IDs
	statSeq := query0.GetAllVehicleStatus()
	responseChan, _ := query0.Execute()
	response := <-responseChan
	statusResponse, ok := response.Get(statSeq)
	if !ok {
		fmt.Print((time.Now()).Format("15:04:05"), " ")
		fmt.Println("No response")
	}
	activeIDs := make([]uint32, 4)
	for i, v := range statusResponse.Vehicles {
		activeIDs[i] = v.VehicleId
		fmt.Print((time.Now()).Format("15:04:05"), " ")
		fmt.Println("Active ID:", v.VehicleId)
	}

	// Get sorties
	var sortie [3]Sortie

	openFile, err := os.Open("sorties.csv")
	if err != nil {
		log.Fatal("Unable to open CSV:", err.Error())
	}

	reader := csv.NewReader(openFile)
	reader.Comment = '#'
	for {
		record, err := reader.Read()
		if err == io.EOF {
			break
		}
		if err != nil {
			log.Fatal("Error reading CSV:", err.Error())
		}

		sortieNum, _ := strconv.Atoi(record[0])
		gpsNum, _ := strconv.Atoi(record[1])
		var tempGPS GPS
		tempGPS.lat, _ = strconv.ParseFloat(record[2], 64)
		tempGPS.lon, _ = strconv.ParseFloat(record[3], 64)
		sortie[sortieNum].gps = append(sortie[sortieNum].gps, tempGPS)
		fmt.Print((time.Now()).Format("15:04:05"), " ")
		fmt.Printf("Drone %d - Waypoint %d: %.4f, %.4f\n", sortieNum, gpsNum, sortie[sortieNum].gps[gpsNum].lat, sortie[sortieNum].gps[gpsNum].lon)
	}

	// ASSUMING NO FUEL UP

	// Set bases
	var BASE [3]GPS
	BASE[BABY].lat = 50.3335
	BASE[BABY].lon = -60.6990
	BASE[FRANCIS].lat = 50.5580
	BASE[FRANCIS].lon = -61.0745
	BASE[JOHNNY].lat = 49.6165
	BASE[JOHNNY].lon = -67.7075

	// Launch Drones

	// Goroutine Johnny
	go func(sortie [3]Sortie) {
		fmt.Println((time.Now()).Format("15:04:05"), "GO JOHNNY")
		launch(activeIDs[JOHNNY])
		for _, dest := range sortie[JOHNNY].gps {
			//AddCmd(command list, command type, latitude, longitude, altitude, velocity, turn radius, linger time, transit heading)
			cmdList := icarus.AddCmd(nil, icarus.GOTO, dest.lat, dest.lon, ALT_DIF+ALT_DIF*JOHNNY, MAX_VEL, 0, 0, 0)
			gotoSeq := query0.Goto(int(activeIDs[JOHNNY]), cmdList)
			responseChan, _ = query0.Execute()
			response = <-responseChan
			_, ok = response.Get(gotoSeq)
			if !ok {
				fmt.Print((time.Now()).Format("15:04:05"), " ")
				fmt.Println("No response")
				os.Exit(1)
			}

			fmt.Print((time.Now()).Format("15:04:05"), " ")
			fmt.Printf("Moving vehicle %d to %.4f %d %.4f at %.0f meters/sec.\n", activeIDs[JOHNNY], dest.lat, ALT_DIF+ALT_DIF*JOHNNY, dest.lon, MAX_VEL)

			// Trigger when in range
			var inRange bool = false
			for !inRange {
				time.Sleep(10 * time.Second)
				statSeq := query0.GetVehicleStatus(int(activeIDs[JOHNNY]))
				responseChan, _ := query0.Execute()
				response := <-responseChan
				statusResponse, ok := response.Get(statSeq)
				if !ok {
					fmt.Print((time.Now()).Format("15:04:05"), " ")
					fmt.Println("No response")
					os.Exit(1)
				}
				v := statusResponse.Vehicles[0]
				if v.Telem.Latitude > dest.lat-.0020 && v.Telem.Latitude < dest.lat+.0020 && v.Telem.Longitude > dest.lon-.0020 && v.Telem.Longitude < dest.lon+.0020 {
					inRange = true
				}
			}

			// Take picture
			configs := query0.ExecutePayload(int(activeIDs[JOHNNY]), 4, 1, nil, 0)
			responseChan, _ = query0.Execute()
			fmt.Print((time.Now()).Format("15:04:05"), " ")
			fmt.Printf("Vehicle %v taking picture.\n", activeIDs[JOHNNY])
			response = <-responseChan

			configResponse, ok := response.Get(configs)
			if !ok {
				fmt.Print((time.Now()).Format("15:04:05"), " ")
				fmt.Println("Error during picture taken: \n", configResponse.Message)
			} else {
				fmt.Print((time.Now()).Format("15:04:05"), " ")
				fmt.Printf("Image returned by vehicle %v: %v\n", activeIDs[JOHNNY], configResponse.PayloadResponse.File)
			}

			// Check radar
			statSeq := query0.GetVehicleStatus(int(activeIDs[JOHNNY]))
			responseChan, _ := query0.Execute()
			response := <-responseChan
			statusResponse, ok := response.Get(statSeq)
			if !ok {
				fmt.Println("No response")
				os.Exit(1)
			}
			v := statusResponse.Vehicles[0]

			// Output radar
			fmt.Println("\nPrint radar info:")
			radarInfo := v.PayStatus[icarus.AllRadar]
			//Radar's current range is found in radarInfo.Resources parameter
			if len(radarInfo.Radar) > 0 {
				for asset, radarPing := range radarInfo.Radar {
					//ID 0 means nothing was returned by the radar payload
					if asset != 0 {
						if radarPing.StructureType == 0 {
							fmt.Printf("Radar found vehicle(%d) at (%f,%f,%f)\n", asset, radarPing.Latitude, radarPing.Longitude, radarPing.Altitude)
						} else {
							fmt.Printf("Radar found infrastructure(%d) at (%f,%f,%f)\n", asset, radarPing.Latitude, radarPing.Longitude, radarPing.Altitude)
						}
					}
				}
			}

		}

		// Go back to base
		cmdList := icarus.AddCmd(nil, icarus.GOTO, BASE[JOHNNY].lat, BASE[JOHNNY].lon, ALT_DIF+ALT_DIF*JOHNNY, MAX_VEL, 0, 0, 0)
		gotoSeq := query0.Goto(int(activeIDs[JOHNNY]), cmdList)
		responseChan, _ = query0.Execute()
		response = <-responseChan
		_, ok = response.Get(gotoSeq)
		if !ok {
			fmt.Print((time.Now()).Format("15:04:05"), " ")
			fmt.Println("No response")
			os.Exit(1)
		}

		fmt.Print((time.Now()).Format("15:04:05"), " ")
		fmt.Printf("Moving vehicle %d to base at %.4f %d %.4f at %.0f meters/sec.\n", activeIDs[JOHNNY], BASE[JOHNNY].lat, ALT_DIF+ALT_DIF*JOHNNY, BASE[JOHNNY].lon, MAX_VEL)

		// Trigger when in range
		var inRange bool = false
		for !inRange {
			time.Sleep(10 * time.Second)
			statSeq := query0.GetVehicleStatus(int(activeIDs[JOHNNY]))
			responseChan, _ := query0.Execute()
			response := <-responseChan
			statusResponse, ok := response.Get(statSeq)
			if !ok {
				fmt.Print((time.Now()).Format("15:04:05"), " ")
				fmt.Println("No response")
				os.Exit(1)
			}
			v := statusResponse.Vehicles[0]
			if v.Telem.Latitude > BASE[JOHNNY].lat-.0100 && v.Telem.Latitude < BASE[JOHNNY].lat+.0100 && v.Telem.Longitude > BASE[JOHNNY].lon-.0100 && v.Telem.Longitude < BASE[JOHNNY].lon+.0100 {
				inRange = true
			}
		}

		// Land
		landSeq := query0.SetNavMode(int(activeIDs[JOHNNY]), icarus.LAND_NOW)
		responseChan, _ := query0.Execute()
		response := <-responseChan
		_, ok = response.Get(landSeq)
		if !ok {
			fmt.Print((time.Now()).Format("15:04:05"), " ")
			fmt.Println("No response")
			os.Exit(1)
		}
		fmt.Print((time.Now()).Format("15:04:05"), " ")
		fmt.Printf("Landing vehicle %d.\n", activeIDs[JOHNNY])
	}(sortie)

	// Wait time for next drone
	//time.Sleep(24 * time.Minute)

	// Goroutine Francis
	go func(sortie [3]Sortie) {
		fmt.Println((time.Now()).Format("15:04:05"), "GO FRANCIS")
		launch(activeIDs[FRANCIS])
		for _, dest := range sortie[FRANCIS].gps {
			//AddCmd(command list, command type, latitude, longitude, altitude, velocity, turn radius, linger time, transit heading)
			cmdList := icarus.AddCmd(nil, icarus.GOTO, dest.lat, dest.lon, ALT_DIF+ALT_DIF*FRANCIS, MAX_VEL, 0, 0, 0)
			gotoSeq := query1.Goto(int(activeIDs[FRANCIS]), cmdList)
			responseChan, _ = query1.Execute()
			response = <-responseChan
			_, ok = response.Get(gotoSeq)
			if !ok {
				fmt.Print((time.Now()).Format("15:04:05"), " ")
				fmt.Println("No response")
				os.Exit(1)
			}

			fmt.Print((time.Now()).Format("15:04:05"), " ")
			fmt.Printf("Moving vehicle %d to %.4f %d %.4f at %.0f meters/sec.\n", activeIDs[FRANCIS], dest.lat, ALT_DIF+ALT_DIF*FRANCIS, dest.lon, MAX_VEL)

			// Trigger when in range
			var inRange bool = false
			for !inRange {
				time.Sleep(10 * time.Second)
				statSeq := query1.GetVehicleStatus(int(activeIDs[FRANCIS]))
				responseChan, _ := query1.Execute()
				response := <-responseChan
				statusResponse, ok := response.Get(statSeq)
				if !ok {
					fmt.Print((time.Now()).Format("15:04:05"), " ")
					fmt.Println("No response")
					os.Exit(1)
				}
				v := statusResponse.Vehicles[0]
				if v.Telem.Latitude > dest.lat-.0020 && v.Telem.Latitude < dest.lat+.0020 && v.Telem.Longitude > dest.lon-.0020 && v.Telem.Longitude < dest.lon+.0020 {
					inRange = true
				}
			}

			// Take picture
			configs := query1.ExecutePayload(int(activeIDs[FRANCIS]), 4, 1, nil, 0)
			responseChan, _ = query1.Execute()
			fmt.Print((time.Now()).Format("15:04:05"), " ")
			fmt.Printf("Vehicle %v taking picture.\n", activeIDs[FRANCIS])
			response = <-responseChan

			configResponse, ok := response.Get(configs)
			if !ok {
				fmt.Print((time.Now()).Format("15:04:05"), " ")
				fmt.Println("Error during picture taken: \n", configResponse.Message)
			} else {
				fmt.Print((time.Now()).Format("15:04:05"), " ")
				fmt.Printf("Image returned by vehicle %v: %v\n", activeIDs[FRANCIS], configResponse.PayloadResponse.File)
			}

			// Check radar
			statSeq := query1.GetVehicleStatus(int(activeIDs[FRANCIS]))
			responseChan, _ := query1.Execute()
			response := <-responseChan
			statusResponse, ok := response.Get(statSeq)
			if !ok {
				fmt.Println("No response")
				os.Exit(1)
			}
			v := statusResponse.Vehicles[0]

			// Output radar
			fmt.Println("\nPrint radar info:")
			radarInfo := v.PayStatus[icarus.AllRadar]
			//Radar's current range is found in radarInfo.Resources parameter
			if len(radarInfo.Radar) > 0 {
				for asset, radarPing := range radarInfo.Radar {
					//ID 0 means nothing was returned by the radar payload
					if asset != 0 {
						if radarPing.StructureType == 0 {
							fmt.Printf("Radar found vehicle(%d) at (%f,%f,%f)\n", asset, radarPing.Latitude, radarPing.Longitude, radarPing.Altitude)
						} else {
							fmt.Printf("Radar found infrastructure(%d) at (%f,%f,%f)\n", asset, radarPing.Latitude, radarPing.Longitude, radarPing.Altitude)
						}
					}
				}
			}

		}
		// Go back to base
		cmdList := icarus.AddCmd(nil, icarus.GOTO, BASE[FRANCIS].lat, BASE[FRANCIS].lon, ALT_DIF+ALT_DIF*FRANCIS, MAX_VEL, 0, 0, 0)
		gotoSeq := query1.Goto(int(activeIDs[FRANCIS]), cmdList)
		responseChan, _ = query1.Execute()
		response = <-responseChan
		_, ok = response.Get(gotoSeq)
		if !ok {
			fmt.Print((time.Now()).Format("15:04:05"), " ")
			fmt.Println("No response")
			os.Exit(1)
		}

		fmt.Print((time.Now()).Format("15:04:05"), " ")
		fmt.Printf("Moving vehicle %d to base at %.4f %d %.4f at %.0f meters/sec.\n", activeIDs[FRANCIS], BASE[FRANCIS].lat, ALT_DIF+ALT_DIF*FRANCIS, BASE[FRANCIS].lon, MAX_VEL)

		// Trigger when in range
		var inRange bool = false
		for !inRange {
			time.Sleep(10 * time.Second)
			statSeq := query1.GetVehicleStatus(int(activeIDs[FRANCIS]))
			responseChan, _ := query1.Execute()
			response := <-responseChan
			statusResponse, ok := response.Get(statSeq)
			if !ok {
				fmt.Print((time.Now()).Format("15:04:05"), " ")
				fmt.Println("No response")
				os.Exit(1)
			}
			v := statusResponse.Vehicles[0]
			if v.Telem.Latitude > BASE[FRANCIS].lat-.0100 && v.Telem.Latitude < BASE[FRANCIS].lat+.0100 && v.Telem.Longitude > BASE[FRANCIS].lon-.0100 && v.Telem.Longitude < BASE[FRANCIS].lon+.0100 {
				inRange = true
			}
		}

		// Land
		landSeq := query1.SetNavMode(int(activeIDs[FRANCIS]), icarus.LAND_NOW)
		responseChan, _ := query1.Execute()
		response := <-responseChan
		_, ok = response.Get(landSeq)
		if !ok {
			fmt.Print((time.Now()).Format("15:04:05"), " ")
			fmt.Println("No response")
			os.Exit(1)
		}
		fmt.Print((time.Now()).Format("15:04:05"), " ")
		fmt.Printf("Landing vehicle %d.\n", activeIDs[FRANCIS])
	}(sortie)

	// Wait time for next drone
	//time.Sleep(38 * time.Minute)

	// Goroutine BABY
	go func(sortie [3]Sortie) {
		fmt.Println((time.Now()).Format("15:04:05"), "GO BABY")
		launch(activeIDs[BABY])
		for _, dest := range sortie[BABY].gps {
			//AddCmd(command list, command type, latitude, longitude, altitude, velocity, turn radius, linger time, transit heading)
			cmdList := icarus.AddCmd(nil, icarus.GOTO, dest.lat, dest.lon, ALT_DIF+ALT_DIF*BABY, MAX_VEL, 0, 0, 0)
			gotoSeq := query2.Goto(int(activeIDs[BABY]), cmdList)
			responseChan, _ = query2.Execute()
			response = <-responseChan
			_, ok = response.Get(gotoSeq)
			if !ok {
				fmt.Print((time.Now()).Format("15:04:05"), " ")
				fmt.Println("No response")
				os.Exit(1)
			}

			fmt.Print((time.Now()).Format("15:04:05"), " ")
			fmt.Printf("Moving vehicle %d to %.4f %d %.4f at %.0f meters/sec.\n", activeIDs[BABY], dest.lat, ALT_DIF+ALT_DIF*BABY, dest.lon, MAX_VEL)

			// Trigger when in range
			var inRange bool = false
			for !inRange {
				time.Sleep(10 * time.Second)
				statSeq := query2.GetVehicleStatus(int(activeIDs[BABY]))
				responseChan, _ := query2.Execute()
				response := <-responseChan
				statusResponse, ok := response.Get(statSeq)
				if !ok {
					fmt.Print((time.Now()).Format("15:04:05"), " ")
					fmt.Println("No response")
					os.Exit(1)
				}
				v := statusResponse.Vehicles[0]
				if v.Telem.Latitude > dest.lat-.0020 && v.Telem.Latitude < dest.lat+.0020 && v.Telem.Longitude > dest.lon-.0020 && v.Telem.Longitude < dest.lon+.0020 {
					inRange = true
				}
			}

			// Take picture
			configs := query2.ExecutePayload(int(activeIDs[BABY]), 4, 1, nil, 0)
			responseChan, _ = query2.Execute()
			fmt.Print((time.Now()).Format("15:04:05"), " ")
			fmt.Printf("Vehicle %v taking picture.\n", activeIDs[BABY])
			response = <-responseChan

			configResponse, ok := response.Get(configs)
			if !ok {
				fmt.Print((time.Now()).Format("15:04:05"), " ")
				fmt.Println("Error during picture taken: \n", configResponse.Message)
			} else {
				fmt.Print((time.Now()).Format("15:04:05"), " ")
				fmt.Printf("Image returned by vehicle %v: %v\n", activeIDs[BABY], configResponse.PayloadResponse.File)
			}

			// Check radar
			statSeq := query2.GetVehicleStatus(int(activeIDs[BABY]))
			responseChan, _ := query2.Execute()
			response := <-responseChan
			statusResponse, ok := response.Get(statSeq)
			if !ok {
				fmt.Println("No response")
				os.Exit(1)
			}
			v := statusResponse.Vehicles[0]

			// Output radar
			fmt.Println("\nPrint radar info:")
			radarInfo := v.PayStatus[icarus.AllRadar]
			//Radar's current range is found in radarInfo.Resources parameter
			if len(radarInfo.Radar) > 0 {
				for asset, radarPing := range radarInfo.Radar {
					//ID 0 means nothing was returned by the radar payload
					if asset != 0 {
						if radarPing.StructureType == 0 {
							fmt.Printf("Radar found vehicle(%d) at (%f,%f,%f)\n", asset, radarPing.Latitude, radarPing.Longitude, radarPing.Altitude)
						} else {
							fmt.Printf("Radar found infrastructure(%d) at (%f,%f,%f)\n", asset, radarPing.Latitude, radarPing.Longitude, radarPing.Altitude)
						}
					}
				}
			}

		}
		// Go back to base
		cmdList := icarus.AddCmd(nil, icarus.GOTO, BASE[BABY].lat, BASE[BABY].lon, ALT_DIF+ALT_DIF*BABY, MAX_VEL, 0, 0, 0)
		gotoSeq := query2.Goto(int(activeIDs[BABY]), cmdList)
		responseChan, _ = query2.Execute()
		response = <-responseChan
		_, ok = response.Get(gotoSeq)
		if !ok {
			fmt.Print((time.Now()).Format("15:04:05"), " ")
			fmt.Println("No response")
			os.Exit(1)
		}

		fmt.Print((time.Now()).Format("15:04:05"), " ")
		fmt.Printf("Moving vehicle %d to base at %.4f %d %.4f at %.0f meters/sec.\n", activeIDs[BABY], BASE[BABY].lat, ALT_DIF+ALT_DIF*BABY, BASE[BABY].lon, MAX_VEL)

		// Trigger when in range
		var inRange bool = false
		for !inRange {
			time.Sleep(10 * time.Second)
			statSeq := query2.GetVehicleStatus(int(activeIDs[BABY]))
			responseChan, _ := query2.Execute()
			response := <-responseChan
			statusResponse, ok := response.Get(statSeq)
			if !ok {
				fmt.Print((time.Now()).Format("15:04:05"), " ")
				fmt.Println("No response")
				os.Exit(1)
			}
			v := statusResponse.Vehicles[0]
			if v.Telem.Latitude > BASE[BABY].lat-.0100 && v.Telem.Latitude < BASE[BABY].lat+.0100 && v.Telem.Longitude > BASE[BABY].lon-.0100 && v.Telem.Longitude < BASE[BABY].lon+.0100 {
				inRange = true
			}
		}

		// Land
		landSeq := query2.SetNavMode(int(activeIDs[BABY]), icarus.LAND_NOW)
		responseChan, _ := query2.Execute()
		response := <-responseChan
		_, ok = response.Get(landSeq)
		if !ok {
			fmt.Print((time.Now()).Format("15:04:05"), " ")
			fmt.Println("No response")
			os.Exit(1)
		}
		fmt.Print((time.Now()).Format("15:04:05"), " ")
		fmt.Printf("Landing vehicle %d.\n", activeIDs[BABY])
	}(sortie)

	time.Sleep(2 * time.Hour)

}

func launch(vehID uint32) {

	navSeq := query0.SetNavMode(int(vehID), icarus.NAVIGATION) // Drones can go straight into navigation mode; may be issue in future
	responseChan, _ := query0.Execute()
	response := <-responseChan
	_, ok := response.Get(navSeq)
	if !ok {
		fmt.Print((time.Now()).Format("15:04:05"), " ")
		fmt.Println("No response")
	}
	fmt.Print((time.Now()).Format("15:04:05"), " ")
	fmt.Printf("Launching vehicle %d.\n", vehID)
}

func addVehicles() {
	openFile, err := os.Open("assets.csv")
	if err != nil {
		log.Fatal("Unable to open CSV:", err.Error())
	}

	reader := csv.NewReader(openFile)
	reader.Comment = '#'
	reader.Read()
	for {
		record, err := reader.Read()
		if err == io.EOF {
			break
		}
		if err != nil {
			log.Fatal("Error reading CSV:", err.Error())
		}

		secMode, _ := strconv.Atoi(record[6])
		secMap := make([]string, 8)
		secMap[0] = ""
		secMap[1] = record[7]
		secMap[2] = record[8]
		secMap[3] = record[9]
		secMap[4] = record[10]
		secMap[5] = record[11]
		secMap[6] = record[12]
		secMap[7] = record[13]

		// Add new vehicle to IcarusServer
		addSeq := query0.AddNewVehicle(record[2], record[3], record[1], record[4], secMode, secMap, []byte(defaultCert), []byte(defaultKey), 1, icarus.DefaultC3poTime, icarus.DefaultDaedalusTime)
		responseChan, _ := query0.Execute()
		response := <-responseChan
		fmt.Print((time.Now()).Format("15:04:05"), " ")
		fmt.Println(response.Get(addSeq))
	}
	fmt.Print((time.Now()).Format("15:04:05"), " ")
	fmt.Println("All vehicles added.")
}

func status_all() {
	statSeq := query0.GetAllVehicleStatus()
	responseChan, _ := query0.Execute()
	response := <-responseChan
	statusResponse, ok := response.Get(statSeq)
	if !ok {
		fmt.Print((time.Now()).Format("15:04:05"), " ")
		fmt.Println("No response")
	}

	//Output
	fmt.Print((time.Now()).Format("15:04:05"), " ")
	fmt.Println("All Vehicles Status:")
	for _, v := range statusResponse.Vehicles {
		fmt.Print("=---= ")
		fmt.Print(v.VehicleCallsign, " [", v.VehicleId, "] // Team # ", v.VConfig.TeamId, " =-----------------------=\n")
		if !v.Available {
			println("DESTROYED\n")
		} else {
			fmt.Printf("Coordinates:\t%.4f, %.0f, %.4f", v.Telem.Latitude, v.Telem.Altitude, v.Telem.Longitude)
			fmt.Print("\nVector:\t\t", v.Telem.Heading, "°") // This is broken
			switch d := v.Telem.Heading; {
			case d < 22.5:
				fmt.Print("N")
			case d < 67.5:
				fmt.Print("NE")
			case d < 112.5:
				fmt.Print("E")
			case d < 157.5:
				fmt.Print("SE")
			case d < 202.5:
				fmt.Print("S")
			case d < 227.5:
				fmt.Print("SW")
			case d < 292.5:
				fmt.Print("W")
			case d < 327.5:
				fmt.Print("NW")
			case d >= 327.5:
				fmt.Print("N")
			default:
			}
			fmt.Printf("\t%.0f m/s", v.Telem.Velocity)
			fmt.Print("\nFuel:\t\t", v.PayStatus[5].Resources)
			// fmt.Print("\nValid:\t\t", v.VehicleType.Valid) Find out what this is
			fmt.Print("\nNav Mode:\t", v.VehicleType.Mode)
			switch v.VehicleType.Mode {
			case 0:
				fmt.Print(" (NONE)\n")
			case 1:
				fmt.Print(" (TAKE_OFF)\n")
			case 2:
				fmt.Print(" (HOME)\n")
			case 3:
				fmt.Print(" (RALLY)\n")
			case 4:
				fmt.Print(" (LINGER_NOW)\n")
			case 5:
				fmt.Print(" (LINGER_WAYPOINT)\n")
			case 6:
				fmt.Print(" (FOLLOW_THE_LEADER)\n")
			case 7:
				fmt.Print(" (NAVIGATION)\n")
			case 8:
				fmt.Print(" (LAND_NOW)\n")
			case 9:
				fmt.Print(" (LAND_WAYPOINT)\n")
			case 10:
				fmt.Print(" (MANUAL_CONTROL)\n")
			}
		}
	}
}
